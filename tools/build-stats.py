...
# Icons
#
# Inline SVG, not emoji. QuickImageViewer's strip uses emoji; this one does not,
# for the same reason its navigation stopped: an emoji is drawn by whoever made
# the font, so a row of six is a row of six illustration styles that never match
# each other or anything else on the page.
#
# 24x24 line art in the same family as the nav icons and the hero glyph row, so
# the whole site draws from one set. currentColor throughout, so hover recolours
# them for free.
# --------------------------------------------------------------------------
...
ICON = {
    ...
    'star': _svg('<path d="m12 3.5 2.6 5.4 5.9.8-4.3 4.1 1.1 5.9-5.3-2.9-5.3 2.9 1.1-5.9L3.5 9.7'
                 'l5.9-.8z"/>'),
    # A release tag.
...
FIXED = [
    ('image',    '49',  'image formats',
     'https://icyhoty2k.github.io/QuickImageViewer/#format-support'),
    ('keyboard', '224', 'keyboard shortcuts',
     'https://icyhoty2k.github.io/QuickImageViewer/shortcuts.html'),
    ('code',     '60k', 'lines of C++',
...
def render(live):
    rows = [
        ('download', humanise(live['downloads']), 'downloads',
         'https://github.com/icyhoty2k/QuickImageViewer/releases'),
        ('star', humanise(live['stars']), 'stars',
         'https://github.com/icyhoty2k/QuickImageViewer/stargazers'),
        ('tag', humanise(live['releases']), 'releases',
         'https://github.com/icyhoty2k/QuickImageViewer/releases'),
    ] + FIXED
    ...
    out = [START,
           ...
           '<div class="statstrip">']
    for icon, value, label, href in rows:
